...
| DTO | Purpose | Fields và validation | Relationships | Ownership |
|---|---|---|---|---|
| `KnowledgeSearchRequest` | Truy xuất xác định trước | `query` 1..4000; `domain_filter[]` thuộc 7 domain; `top_k` 1..20, mặc định 5; `score_threshold` 0..1, mặc định theo config (architecture nêu 0.7) | KnowledgeDomain | RAG Engine |
| `KnowledgeSearchResponse` | Kết quả xếp hạng | `chunks[]`; `has_results`; `has_sufficient_info`; `conflict_detected`; `search_metadata` | KnowledgeChunk | Knowledge service |
...
|---|---|---|---|---|---|---|---|
| `search_knowledge_base` | RAG nguồn chính thức | `query, domain_filter, top_k, score_threshold` → chunks, sufficient/conflict flags | PC-01; hỗ trợ PC-03/04; CAP-2 | RAG Engine; FND-KNW-01 | `KNOWLEDGE_UNAVAILABLE`, `NO_GROUNDED_RESULT`, `CONTENT_CONFLICT` | 1 lần cho lỗi tạm thời; không retry empty/conflict | 800ms |
...
sequenceDiagram
    ...
    alt Critical
        ...
        F->>E: Embed + domain-filtered search
        E-->>F: Ranked approved chunks
        F-->>T: KnowledgeSearchResponse
