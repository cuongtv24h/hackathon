...
| SBG-01, SBG-03 | JTBD-02 Biết đúng kênh đặt lịch | BF-1; BF-3 Đặt lịch theo Architecture | PC-03 | Appointment Booking | read tools, `create_appointment_draft`, `submit_appointment_request`, `fallback_response` | Specialty, Doctor, Slot, Draft/Request, Channel Config | Shared LangGraph Agent, Appointment Service, HIS Adapter | AC-P05, AC-T01, AC-T02, AC-T03 |
| SBG-01, SBG-03 | JTBD-03 Xác nhận lịch hẹn | BF-4 Tra cứu lịch | PC-04 | Appointment Status | `get_patient_appointments`, `get_appointment_request_status`, `fallback_response` | Authorized Appointment/Request Lookup | Shared LangGraph Agent, Appointment Service, HIS Adapter | AC-P05, AC-P07, AC-T01, AC-T03 |
...
| SBG-01, SBG-03 | JTBD-08 Bước tiếp theo sau khám | BF-1; BF-4 nếu có mã hẹn | Model chọn tool trong authorized subset | Shared Agent Profile | `search_knowledge_base`, authorized status tools, `fallback_response` | Knowledge Search, Appointment Lookup | Shared LangGraph Agent, RAG Engine, Appointment Service | AC-P01, AC-P05, AC-P07 |
...
```json
{
  "trace_id": "trc-03",
  "capability": "PC-03",
  "outcome": "request_pending_staff_review",
  "result": {"request_id": "REQ-2026-0342", "status": "pending_staff_review", "next_step": "Chờ bệnh viện xác nhận"},
  "flow_state": {"flow_id": "flw-01", "step": "pending_staff_review", "missing_fields": []}
}
```
...
| `AppointmentBookingResponse` | Bước/kết quả booking | `outcome=collecting_information|confirmation_required|request_pending_staff_review|cancelled|redirected|unavailable`; `flow_state`; `prompt`; `options[]`; draft/request tùy outcome | BookingFlowState, AppointmentDraft, AppointmentRequest | PC-03 |
...
AI nhận `message`, tối đa 20 lượt `ConversationContext`, `BusinessContext`, `caution_flags`, trusted runtime context và validated observations. Input phải qua validation, rate limit, prompt-injection detection và emergency gate trước graph. Không đưa raw PII vào log/prompt ngoài mục đích booking được phê duyệt.

## 5.2 Output Contract

Output có cấu trúc gồm `agent_decision`, `tool_policy_decisions[]`, `observation_results[]`, `conversation_result`, `explainability_result`, `safety`, `citations[]`, `suggested_actions[]` và optional interrupt. Text gửi người dùng chỉ được dựng từ observation hợp lệ hoặc template đã duyệt. Không trả hoặc checkpoint chain-of-thought, prompt hệ thống, token bí mật hay dữ liệu của người khác.

## 5.3 Agent and policy decisions

`AgentDecisionDTO` ghi next action `tool_call|clarify|final|abstain`, normalized tool calls, public reason code, safety disposition và budget state. Nó không phải intent router và không chứa chain-of-thought. `ToolPolicyDecisionDTO` do backend tạo với `allow|deny|confirmation_required`, validated arguments và workflow transition.

## 5.4 Bounded execution rules

1. Emergency có ưu tiên tuyệt đối.
2. Backend cung cấp visible tool subset từ authorization + workflow state + system policy; model tự chọn next tool.
3. Thông tin thực tế cần validated observation hoặc approved static response trước synthesis.
4. Tool được kiểm tra policy/schema lại ngay trước execution.
5. `submit_appointment_request` cần resumable confirmation và idempotency; model không thể tự cấp quyền.
6. Khi thiếu dữ liệu, hỏi làm rõ tối thiểu; không tự điền; dừng khi hết budget/deadline.
...
| `create_appointment_draft` | Tạo/cập nhật draft | collected data → draft + confirmation summary | PC-03; CAP-4 | FND-APT-04 | `INVALID_STATE`, `SLOT_UNAVAILABLE` | Idempotent draft update | 1000ms |
| `submit_appointment_request` | Gửi request sau confirmation | confirmed draft + idempotency → pending_staff_review | PC-03; CAP-4 | FND-APT-05 | `CONFIRMATION_REQUIRED`, `INVALID_STATE`, `DUPLICATE_REQUEST`, `INTEGRATION_UNAVAILABLE` | Cùng key, tối đa 1 | 1500ms |
| `get_patient_appointments` / `get_appointment_request_status` | Tra cứu được authorize | authenticated context + reference → authorized result/null | PC-04; CAP-4 | FND-APT-06 | `UNAUTHENTICATED`, `FORBIDDEN`, `INVALID_REFERENCE`, `INTEGRATION_UNAVAILABLE` | 1 lỗi tạm thời | 1000ms |
...
- Tool visibility không dựa trên intent; policy được kiểm tra lại trước execution và staff-only tools không bao giờ visible cho patient agent.
...
sequenceDiagram
    ...
    AI->>T: create_appointment_draft
    T-->>AI: Draft + public confirmation summary
    AI-->>API: confirmation_required + opaque thread/run reference
    API-->>FE: Màn hình xác nhận
    U->>FE: Xác nhận rõ ràng
    FE->>API: Resume approve + confirmation_token
    API->>AI: Resume LangGraph checkpoint after ownership check
    AI->>T: submit_appointment_request + idempotency key
    T->>F: AppointmentRequestSubmitRequest
    F->>HIS: Create pending staff-review request
    HIS-->>F: request_id + pending_staff_review
    F-->>T: AppointmentRequestDTO
    T-->>API: Result
    API-->>FE: request_pending_staff_review
    FE-->>U: Mã yêu cầu + bước tiếp theo
...
sequenceDiagram
    ...
    participant AI as LLM Orchestrator
    participant T as authorized status tools
    participant F as Appointment Foundation API
    participant HIS as HIS Adapter / External HIS

    U->>FE: Nhập mã hẹn
    FE->>API: AppointmentStatusRequest
    API->>AI: Authenticated context + reference
    AI->>T: get_patient_appointments / get_appointment_request_status
    T->>F: GET appointment
