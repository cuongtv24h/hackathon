...
- `create_appointment_draft` is workflow-scoped; `submit_appointment_request` requires resumed patient confirmation and the same idempotency key for allowed retry. It returns pending staff review, never confirmed.
