...
def lexical_search(cur, query_text: str, limit: int = 5) -> List[SearchCandidateDTO]:
    """Perform lexical search over knowledge chunks using simple tsquery."""
    query = """
        SELECT 
            kc.chunk_id,
            kc.content,
            ...
            kc.metadata,
            kd.domain_code,
            ts_rank(kc.search_document, plainto_tsquery('simple', %s)) as score
        ...
        WHERE kc.is_active = true
          ...
          AND kc.search_document @@ plainto_tsquery('simple', %s)
        ORDER BY score DESC
        LIMIT %s
    """
